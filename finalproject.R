...
# 輸入帳號
username = "wespft.jl_sherry"  # <username here>
...
#輸入密碼
password = "flute0312"  # <password here>
...
for(k in 1:length(NewTag)){
  remDr$navigate(paste0("https://www.instagram.com/",NewTag[k],"/?hl=zh-tw"))
  for(i in 1:5){      
    remDr$executeScript(paste("scroll(0,",i*10000,");"))
    Sys.sleep(5)
    page1_source<-remDr$getPageSource()
...
DATA3<-data.frame(PostUrl=AllPostUrl,Poster=AllPoster,Date=AllDate,
                  Time=AllTime,Post=AllPost,Tag=AllTag,Tagpeople=AllTagpeople,
                  stringsAsFactors = F)
...
ㄈ#data <- mongo(collection="articles",db="test",url = URL)
#data$find()
